...
void Java_ffpetrovic_anrdroid_1aubio_1example_MainActivity_initTempo(JNIEnv *env, jobject obj,
                                                                     jint sampleRate,
                                                                     jint bufferSize) {
    unsigned int win_s = (unsigned int) bufferSize; // window size
    unsigned int hop_s = win_s / 4; // hop size
...
jfloat Java_ffpetrovic_anrdroid_1aubio_1example_MainActivity_getTempo(JNIEnv *env, jobject obj,
                                                                      jfloatArray inputArray) {
    aubio_tempo_t *o = (aubio_tempo_t *) (*env)->GetLongField(env, obj, getPtrFieldId(env, obj));
    fvec_t *input = (fvec_t *) (*env)->GetLongField(env, obj, getInputFieldId(env, obj));
    fvec_t *tempo = (fvec_t *) (*env)->GetLongField(env, obj,
                                                    getTempoFieldId(env, obj)); // input buffer
